!******************************************
      ...
      subroutine ChuaSystem (nuecdf,T, Y, DY, U, mv, vp, f1, f2)
     ...
     !*****************************************************************
      ...
      integer(kind = 4) :: mv
      DOUBLE PRECISION :: B(37),C(37),DV(37)
      integer :: flag, interv
...
! CNO Matrix
      ...
      !Funciones de membresia que son sustituidas por los vectores obtenidos en Matlab
...
!      M2D = (0.5)*(1+(phiG(Y(1))/d))
      ...
      CALL SPCOEF(mv,vp,f1,B,C,DV,FLAG)
      call SVALUE(mv,vp,f1,B,C,DV,Y(1),INTERV,M1D,FLAG)
      PRINT *,' T =',Y(1),'  M1D =',M1D,'  FLAG =',FLAG

      CALL SPCOEF(mv,vp,f2,B,C,DV,FLAG)
      call SVALUE(mv,vp,f2,B,C,DV,Y(1),INTERV,M2D,FLAG)
      PRINT *,' T =',Y(1),'  M2D =',M2D,'  FLAG =',FLAG
!      write(*,*) M1D
!	  write(*,*) M2D
