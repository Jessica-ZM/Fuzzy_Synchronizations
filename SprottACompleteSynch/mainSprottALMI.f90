...
program ChaosSprottAHidden

implicit none

external SprottASystem  ! Sistema caotico dinamico
...
! Contador de llamada a la subroutine SprottA
...
open (unit = unidadEsc, file = 'chaosSprottALMI_tesis.txt', status = 'UNKNOWN')
 write(unidadEsc, 5000) t0, (y(j), j = 1, nuecdf), (er(k), k = 1, 3), (u(k), k = 1, 3), 0, contadorEVal ! condiciones iniciales

 ! Ciclo simulacion
do i = 1, noIteraciones
   call GautshiOrden2(SprottASystem, nuecdf, bandera, beta0, beta1, y1, dy1, sy, sdy, t, w, y, h, dy, u)
...
end program ChaosSprottAHidden
