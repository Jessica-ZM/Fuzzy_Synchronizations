...
integer(kind = 4), parameter :: unidadEsc1 = 10
integer(kind = 4), parameter :: unidadLec1 = 11
integer(kind = 4), parameter :: unidadEsc2 = 12

integer(kind = 4) :: i, j, k, puntos, n, noIteraciones
integer(kind = 4) :: m, VecItera(18000), cont = 0, maxitera(1) = 0
...
! vector para leer las Cond. iniciales
double precision :: VecIC(nuIC,nuecdf)
...
open (unit = unidadEsc2, file = 'EvaluationsCompleteChua30IC.txt', status = 'UNKNOWN')


!inicia el ciclo principal de las condiciones iniciales
...
do m = 1, nuIC
    ...
    !open (unit = unidadEsc1, file = 'chaosChuaTest.txt', status = 'UNKNOWN')
    !write(unidadEsc1, 5000) t0, (y(j), j = 1, nuecdf), (er(k), k = 1, 3), (u(k), k = 1, 3), 0, contadorEVal ! condiciones iniciales
    ...
    do i = 1, noIteraciones
        ...
        if (contadorV == n) then
            ...
            !write(unidadEsc1, 5000) t, (y(j), j = 1, nuecdf), (er(k), k = 1, 3), (u(k), k = 1, 3), i, contadorEVal
    ...
    !close(unit = unidadEsc1)  ! cerramos archivo de escritura
