...
!
subroutine GautshiOrden2(dsedo, nuecdf, bandera, beta0, beta1, y1, dy1, sy, sdy, t, w, y, h, dy,u, palpha)
...
real(kind = 8) :: beta1, beta0, h, t, w, nu, nu2, nu4
real(kind = 8) :: y(nuecdf), dy(nuecdf), y1(nuecdf), dy1(nuecdf) !y's variables de estado
                                                                 !dy's derivadas
real(kind = 8) :: sy(nuecdf), sdy(nuecdf) !variables auxiliares de almacenamiento

real(kind = 8) :: u(3), palpha
...
if (bandera == 1) then
   ...
   call dsedo(nuecdf, t, y, dy, u, palpha)

   !Estimanos yn+1 por medio de un RK2: y = yn+1, fn = dy
   call drtrap(dsedo, nuecdf, y1, dy1, t, y, h, dy, u, palpha)
   ...
   call dsedo(nuecdf, t, y, dy, u, palpha)  !dy = fn+1
